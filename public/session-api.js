  ...
  function createSessionApi(fetchImpl) {
    ...
    return {
      ...
      async readMessages(sessionId) {
        const data = await jsonOrThrow(await request(`/api/messages?sessionId=${encodeURIComponent(sessionId)}`));
        return data.messages || [];
      },
      async readProjectDir(sessionId) {
        const data = await jsonOrThrow(await request(`/api/project?sessionId=${encodeURIComponent(sessionId)}`));
        return data.dir || "";
      },
      async updateProjectDir(sessionId, dir) {
        const data = await jsonOrThrow(await request("/api/project", {
          method: "POST",
