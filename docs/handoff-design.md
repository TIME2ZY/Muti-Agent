...
| G5 | 有 block 的 handoff 写入 `captured` memory，完整度只写 quality — **由 Wave M 实现**；本波次回归锁定（finalize 复用同一 capture） |
...
| 条件 | memory |
|------|--------|
| hasBlock && ok | `kind=handoff`, `status=captured`, `metadata.quality.ok=true` |
| hasBlock && !ok | `kind=handoff`, `status=captured`, `metadata.quality.ok=false` |
| emptyPacket | **不写** |
| 同 `capture_key=handoff:{invocationId}:{target}:{blockIndex}` | 幂等，不重复写 |
| 同非空 `supersession_key` | supersede 旧 active；禁止只按 from→to 替代 |

`quality.ok` 只证明字段完整，不证明内容真实或任务完成。只有用户确认、接收 Agent 验证或可验证完成事件才能把 memory 转为 `confirmed`。没有可靠主题键时 `supersession_key=NULL`，宁可保留两条不同任务。

| 波次 | 谁做 |
|------|------|
| **Wave M** | 在 chat-routes 与 callback 现有路由点复用 parser，并调用同一 `memory-service.captureHandoff`（**首次落地**） |
...
| 2026-07-16 | 审查修订：handoff 完整度与 memory 确认状态解耦；采用 capture/supersession key |
