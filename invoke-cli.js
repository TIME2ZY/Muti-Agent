...
const DEFAULT_SESSION_FILE = path.join(process.cwd(), ".invoke-cli-session.json");
const AGENTS = {
  architect: {
    id: "architect",
    label: "Codex",
...
function getSessionFilePath() {
  return process.env.INVOKE_CLI_SESSION_FILE || DEFAULT_SESSION_FILE;
}
...
function sessionKey(cli) {
  return cli.id || cli.name;
}
...
function resolveResumeSession(cli) {
  if (!cli.resume) return cli;

  const sessions = readSessions();
  const key = sessionKey(cli);
  const sessionId = sessions[key] && sessions[key].sessionId;
  if (!sessionId) {
    throw new Error(`No saved session for ${key}. Run once without --resume first.`);
  }
...
function invoke(cli, prompt, options = {}) {
  const config = typeof cli === "string" ? { name: cli } : cli;
  const resolvedCli = resolveResumeSession(config);
  ...
  const startAttempt = () => {
    ...
    rl.on("line", (line) => {
      ...
      if (sessionId) rememberSession(sessionKey(resolvedCli), sessionId);
...
function main() {
  ...
  if (!parsed.prompt) {
    console.error(
      'Usage: node invoke-cli.js [--agent architect|forge|sage|reviewer] [--resume] [--timeout-ms ms] "你好，请用一句话介绍自己"'
    );
    process.exit(1);
  }
