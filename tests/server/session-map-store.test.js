...
test("upsertAgentProviderSession keeps base and worktree provider sessions side by side", () => {
  const sessions = {};
  store.upsertAgentProviderSession(sessions, "codex", "th-base", "base:C:\\proj");
  ...
  assert.equal(sessions.codex.sessionId, "th-wt");
  assert.equal(sessions.codex.workspaceKey, "worktree:C:\\proj.worktrees\\s1");
  assert.equal(sessions.codex.byWorkspace["base:C:\\proj"].sessionId, "th-base");
  assert.equal(
    sessions.codex.byWorkspace["worktree:C:\\proj.worktrees\\s1"].sessionId,
    "th-wt"
  );
});
...
test("resolveResumeSessionId reads the matching workspace slot", () => {
  ...
  assert.equal(store.resolveResumeSessionId(sessions, "codex", "base:C:\\proj"), "th-base");
  assert.equal(
    store.resolveResumeSessionId(sessions, "codex", "worktree:C:\\proj.worktrees\\s1"),
    "th-wt"
  );
  assert.equal(store.resolveResumeSessionId(sessions, "codex", "worktree:C:\\other"), "");
});
...
test("resolveResumeSessionId rejects a session from another provider or model", () => {
  ...
  assert.equal(
    store.resolveResumeSessionId(sessions, "codex", "base:C:\\proj", "codex:gpt-5.6-sol"),
    "th-codex"
  );
  assert.equal(
    store.resolveResumeSessionId(sessions, "codex", "base:C:\\proj", "grok:grok-4.5"),
    ""
  );
});
...
test("resolveResumeSessionId keeps legacy single-slot maps working", () => {
  ...
  assert.equal(
    store.resolveResumeSessionId(legacyWithKey, "codex", "base:C:\\proj"),
    "legacy-base"
  );
  assert.equal(
    store.resolveResumeSessionId(legacyWithKey, "codex", "worktree:C:\\proj.worktrees\\s1"),
    ""
  );
  ...
  assert.equal(
    store.resolveResumeSessionId(legacyNoKey, "codex", "base:C:\\proj"),
    "legacy-plain"
  );
  assert.equal(
    store.resolveResumeSessionId(legacyNoKey, "codex", "worktree:C:\\proj.worktrees\\s1"),
    ""
  );
});
...
test("upsert migrates a legacy single-slot entry into byWorkspace without losing it", () => {
  ...
  assert.equal(sessions.codex.byWorkspace["base:C:\\proj"].sessionId, "legacy-base");
  assert.equal(
    sessions.codex.byWorkspace["worktree:C:\\proj.worktrees\\s1"].sessionId,
    "th-wt"
  );
  assert.equal(store.resolveResumeSessionId(sessions, "codex", "base:C:\\proj"), "legacy-base");
});
