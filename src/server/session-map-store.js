...
function getByWorkspaceMap(entry) {
  ...
  if (entry.byWorkspace && typeof entry.byWorkspace === "object" && !Array.isArray(entry.byWorkspace)) {
    const out = {};
  ...
  if (typeof entry.sessionId === "string" && entry.sessionId) {
    const key = typeof entry.workspaceKey === "string" && entry.workspaceKey
      ? entry.workspaceKey
...
function resolveResumeSessionId(sessionMap, agent, workspaceKey) {
  ...
  if (workspaceKey) {
    ...
    if (direct && typeof direct.sessionId === "string" && direct.sessionId) {
    ...
    if (workspaceKey.startsWith("base:") && byWorkspace[LEGACY_WORKSPACE_KEY]) {
      return byWorkspace[LEGACY_WORKSPACE_KEY].sessionId || "";
    }
    return "";
  }
  ...
  for (const slot of Object.values(byWorkspace)) {
    if (slot && typeof slot.sessionId === "string" && slot.sessionId) {
